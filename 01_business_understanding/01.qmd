...
def cost_tbl(
    ...
    df["email_no_growth"] = np.repeat(email_list_size_1, 12)
    df["email_no_growth"] = np.repeat(email_list_size_1, 12)  # this is email list size during 12 month period
    df["lost_customers_no_growth"] = df["email_no_growth"] * ((1 + 0.035) ** df["period"])
...
df = cost_tbl(email_list_size=50000, sales_emails=1)
```
...
data_dict = {email_gth_rate: np.linspace(0,0.05, num = 10),
  convertion_rate:  np.linspace(0.04, 0.06, num = 10)}
  

data_dict
parameter_grid = jn.expand_grid(others = data_dict)
parameter_grid
...
def temp_func(x,y):
  # TODO: write code...
  cost_df = cost_tbl(email_list_size=x,
  conversion_rate=y)
...
summary_list = [temp_func(x,y) for x,y in zip
( parameter_grid['email_gth_rate'],
  parameter_grid['convertion_rate'])]
